    ...
    draw() {
        ...
        for (let i = 0; i < pathLimit; i++) {
            for (let planet of planets) {
                ...
                nextVelocity.x += 1 / Math.pow(r, 3) * Math.cos(theta) * speed;
                nextVelocity.y += 1 / Math.pow(r, 3) * Math.sin(theta) * speed;
            }
        ...
        ctx.strokeStyle = this.color + "80";        //strokeStyle = fillStyle + alpha = 0.5
        ctx.lineWidth = 2;
        ctx.stroke();
    ...
    update() {
        ...
        if (this.readyToLaunch) {
            ...
            for (let planet of planets) {
                ...
                let r = Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2));
                let theta = Math.atan2(y2 - y1, x2 - x1);
                this.vX += 1 / Math.pow(r, 3) * Math.cos(theta) * speed;
                this.vY += 1 / Math.pow(r, 3) * Math.sin(theta) * speed;
            }
